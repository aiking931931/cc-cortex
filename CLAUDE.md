...
- **What Sancio actually owns** (narrow, on purpose): `PostToolUse` hard deny (CC L6 can only warn), cross-session `state_store`, `subagent_fork` for real-time supervision (CC L1 loses control after spawn), byte-exact cache fork. Sancio **consumes** `concinno-skills-*`; it does not re-implement them.
