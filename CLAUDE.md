...
- **CC (my private workspace `E:\Cursor`)** = the application. Consumes CCC via `pip install concinno`.
...
3. **Fix bugs at the source.** If CCC behavior is wrong, fix it in `src/concinno/` — never monkey-patch inside CC. CC copying CCC logic is a rule violation.
