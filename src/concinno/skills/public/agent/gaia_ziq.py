...
def sps_buffs(q_raw, file_name):
    ...
    if re.search(r"youtube\.com|youtu\.be", q):
        scores["youtube"] = 0.95
    calc_kw = ["calculate","how many","sum","average","ratio","convert","m^3","percentage","multiply"]
...
class FTRLv2:
    def __init__(self, lr=0.3):
        ...
        self.lr = lr
    def gate(self, buff):
        if self.n[buff] == 0: return 0.5
        sign = -1 if self.z[buff] < 0 else 1
        return max(0.01, min(0.99, 0.5 - sign * self.z[buff] / (self.lr + math.sqrt(self.n[buff]))))
    def update(self, buff, reward):
        g = -(reward - 0.5)
        self.z[buff] += g; self.n[buff] += g * g
    def save(self):
        with open(FTRL_PATH, "w") as f: json.dump({"z": self.z, "n": self.n}, f)
    def load(self):
        if os.path.exists(FTRL_PATH):
            d = json.load(open(FTRL_PATH))
            self.z = d.get("z", self.z); self.n = d.get("n", self.n)
...
def solve(question, file_content="", file_name=""):
    ...
    if "youtube" in buffs:
        m = re.search(r"https?://(?:www\.)?(?:youtube\.com|youtu\.be)/\S+", question)
        if m: ctx["youtube"] = get_youtube_transcript(m.group(0))[:3000]
    if "code_exec" in buffs:
        ...
        if cm:
            try:
                r = subprocess.run([sys.executable, "-c", cm.group(1)], capture_output=True, text=True, timeout=10)
...
def train(n="all"):
    ...
    router = FTRLv2(); router.load()
    b.launch(headless=True)
    ok = tot = 0
    for i, task in enumerate(tasks):
        q = task["Question"]; exp = task["Final answer"]; fn = task.get("file_name", "")
        fc = ""
        fp = task.get("file_path", "")
        if fp:
            try: fc = read_file(hf_hub_download("gaia-benchmark/GAIA", fp, repo_type="dataset"))
            except: pass
        sps = sps_buffs(q, fn)
        ...
        if t.is_alive():
            ...
            try: b.close(); b.launch(headless=True)
            except: pass
        ans = box[0]
        match = ans.strip().lower() == exp.strip().lower()
        for bf in buffs_used: router.update(bf, 1.0 if match else 0.0)
        tot += 1; ok += int(match)
        tag = "PASS" if match else "FAIL"
        bstr = "+".join(buffs_used) if buffs_used else "base"
        print(f"[{i}/{len(tasks)}] L{task['Level']} {bstr:25} {tag} got=\"{ans[:25]}\" exp=\"{exp[:25]}\"", flush=True)
        if (i+1) % 20 == 0:
            router.save()
            print(f"  --- {ok}/{tot} ({100*ok//max(tot,1)}%) ---", flush=True)
    b.close(); router.save()
    print(f"\n=== {ok}/{tot} ({100*ok//max(tot,1)}%) ===")
    for bf in BUFFS: print(f"  {bf}: gate={router.gate(bf):.2f}")

if __name__ == "__main__":
    import argparse; ap = argparse.ArgumentParser()
    ap.add_argument("--train", nargs="?", const="20")
    a = ap.parse_args()
    if a.train: train(a.train if a.train != "all" else "all")
