"""concinno.library — CCC as a Python library (non-hook entry point).
...
@dependencies concinno.guards.*, concinno.c0_router, concinno.prompt_engine
...
class CortexLibrary:
    ...
    def _default_cache_dir(self) -> str:
        ...
        if self.workspace:
            d = os.path.join(self.workspace, ".concinno_cache")
        else:
            d = os.path.join(tempfile.gettempdir(), "concinno_cache")
