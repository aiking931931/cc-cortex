"""concinno.hooks.io_utils — Shared I/O helpers for Claude Code hooks.
...
def _is_silent() -> bool:
    ...
    Resolution order:
    1. ``CONCINNO_SILENT`` env var (one-shot CLI override, highest priority)
    2. ``profile_settings.silent`` in cc_config.json (set via /hook profile competition)
    """
    env_val = os.environ.get("CONCINNO_SILENT")
...
def write_json_stdout(data: dict) -> None:
    ...
    In silent mode (CONCINNO_SILENT=1): additionalContext is wrapped
...
def cache_path(*parts: str) -> str:
    """Join parts relative to .concinno_cache/."""
    return project_path(".concinno_cache", *parts)


# ── Timezone ────────────────────────────────────────────────

def get_local_tz():
    """Get local timezone (UTC+8), preferring concinno.core.config if available."""
    try:
        from concinno.core.config import LOCAL_TZ
        return LOCAL_TZ
    except ImportError:
