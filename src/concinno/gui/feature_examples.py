...
EXAMPLES: dict[str, str] = {
    ...
    "gaia_music_image_upscale": (
        ...
        "treble clef / notes / staff) AND has an attached image "
        "below the upscale threshold, run a 4× LANCZOS upscale on "
        "the image before handing it to the local vision encoder. "
        "Pure preprocess — no prompt injection, no answer-path "
        "scaffolding lives under this toggle.\n\n"
        "Renamed from legacy `bassclef_wordreverse` (2026-04-26) "
        "to remove task-specific naming. The legacy key still "
        "works for one minor version via the back-compat alias "
        "(deprecation warning on use).\n\n"
        "Requires: Pillow (PIL) — a dependency of the vision "
        "path, so this is effectively always available. The "
        "L1 procedural anchor (generic music-theory text) is a "
        "separate toggle, `gaia_music_procedure_anchor`."
    ),
    "gaia_polygon_image_upscale": (
        "When a question mentions a polygon / edges / sides / "
        "vertices AND has an attached image below the upscale "
        "threshold, run a 4× LANCZOS upscale on the image before "
        "handing it to the local vision encoder so small numeric "
        "labels survive the rescale. Pure preprocess — no prompt "
        "injection.\n\n"
        "Renamed from legacy `polygon_counting_hint` (2026-04-26) "
        "to remove task-specific naming. The legacy key still "
        "works for one minor version via the back-compat alias "
        "(deprecation warning on use).\n\n"
        "Requires: same as `gaia_music_image_upscale`. The L1 "
        "polygon-area procedural anchor (generic geometry "
        "procedure text) is a separate toggle, "
