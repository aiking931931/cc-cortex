---
name: mode
description: Switch concinno profile between daily / competition / handoff. Competition mode silences advisory guards (CBUA / WIREDO / Read:Edit ratio) while keeping safety guards loud. Triggers on keywords like "mode", "模式", "切換", "profile", "daily", "competition", "handoff".
user-invocable: true
...
Switch the active concinno profile. This is the knob users turn to
...
1. Parse `$ARGUMENTS` — lowercase, trim. Empty / `status` → print
   current profile via `python -c "from concinno.feature_config import get_active_profile; print(get_active_profile())"`.
...
3. Apply it:
   ```bash
   python -c "from concinno.feature_config import apply_profile; print('\n'.join(apply_profile('<canonical>')))"
   ```
4. Optionally export `CONCINNO_PROFILE=<canonical>` in the current
   shell so new subprocesses see the profile without re-reading
   `cc_config.json`.
5. Print confirmation:
   ```
   ⚙ concinno profile: <alias> (<canonical>)
...
- `concinno/feature_config.py::PROFILES` — canonical profile list
- `concinno/feature_config.py::get_active_profile` — resolver
- `concinno/guards/base.py::GuardResult.allow_advisory` — helper
- `concinno/guards/pipeline.py::GuardPipeline._collect_context` — routing
