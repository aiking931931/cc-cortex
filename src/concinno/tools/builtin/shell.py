"""concinno.tools.builtin.shell — Bash execution tool (Aegis P0.2).
...
@dependencies concinno.security.bash_validators (hard),
    concinno.destruction_guard (hard),
    concinno.core.state_store (hard),
...
from concinno.destruction_guard import evaluate as destruction_evaluate
from concinno.security.bash_validators import BashValidator

logger = logging.getLogger("concinno.tools.builtin.shell")
...
def _make_background_log_path(job_id: str) -> Path:
    """Return ``~/.concinno_cache/shell_logs/{job_id}.log``.
    ...
    """
    base = Path(os.environ.get("CONCINNO_CACHE_DIR", str(Path.home() / ".concinno_cache")))
        ...
        *,
        ...
        Args:
            ...
            cache_dir: Base directory for StateStore + background logs.
                Defaults to ``$CONCINNO_CACHE_DIR`` or
                ``~/.concinno_cache``.
        """
        self._validator = validator or BashValidator()
        if cache_dir is None:
            cache_dir = os.environ.get(
                "CONCINNO_CACHE_DIR", str(Path.home() / ".concinno_cache")
            )
