"""concinno.prompt_engine — Dynamic prompt assembly with token budgets.
...
@dependencies concinno.cognitive_inject, concinno.token_zone
...
def _build_ftrl_context(
    ...
    if not os.path.isfile(path):
        return None
    try:
        from concinno.knowledge import ftrl_weight
...
class StaticCache:
    ...
    def load(self, workspace: str) -> None:
        ...
        Override precedence:
          1. ``CONCINNO_IDENTITY_PATH`` env var (explicit file path)
          2. ``CONCINNO_L0_RULES_PATH`` env var (explicit file path)
          3. ``<workspace>/.concinno/identity.md``
        ...
        if not identity_path and workspace:
            cand = os.path.join(workspace, ".concinno", "identity.md")
        ...
        l0_path = os.environ.get("CONCINNO_L0_RULES_PATH", "")
        if not l0_path and workspace:
            for cand in (
                os.path.join(workspace, ".concinno", "l0.md"),
