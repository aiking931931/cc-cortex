"""concinno.cognitive_inject — Three-layer knowledge router.
...
@dependencies concinno.hooks.io_utils
...
def _depth_from_c0(workspace: str) -> str:
    ...
    try:
        ...
        store = StateStore(
            os.path.join(workspace, ".concinno_cache") if workspace else "",
        )
...
def _load_learnings(workspace: str) -> list[dict]:
    ...
    try:
        from concinno.hooks.io_utils import learnings_path
        path = learnings_path()
    except ImportError:
