"""concinno.llm_guard — LLM-backed semantic guards (optional [llm] dep).
...
@responsibility Break the zero-dep ceiling: provide LLM-level semantic
    ...
    offer. Requires ``pip install concinno[llm]``.
...
_DEFAULT_MODEL = os.environ.get(
    "CONCINNO_LLM_MODEL", "claude-haiku-4-5-20251001",
)
_MAX_TOKENS = 256
...
def _call_llm(prompt: str) -> str:
    ...
    try:
        ...
        if key:
            model = os.environ.get("CONCINNO_LLM_MODEL", "gpt-4o-mini")
...
class SemanticInjectionGuard(LLMGuard):
    ...
    that regex patterns miss. Requires ``concinno[llm]``.
    """
