"""concinno.mcp_server -- MCP Server for CC Cortex.
...
Resources:
    concinno://session/status    -- Current session state
    concinno://metrics/quality   -- B4 quality scores
    concinno://metrics/tokens    -- Token usage stats
    concinno://knowledge/stats   -- Knowledge base statistics

Tools:
    concinno-status  -- Full module status dashboard
    concinno-doctor  -- Health check

Usage:
    python -m concinno.mcp_server
"""
...
TOOLS = [
    {
        "name": "concinno-status",
        "description": (
            "Returns full module status dashboard (equivalent to `concinno status` CLI)"
        ),
        "inputSchema": {
            "type": "object",
            "properties": {},
    ...
    {
        "name": "concinno-doctor",
        "description": (
            "Run health check on CC Cortex installation (equivalent to `concinno doctor` CLI)"
        ),
        "inputSchema": {
            "type": "object",
            "properties": {},
    ...
    {
        "name": "concinno-failure-patterns",
        "description": (
    ...
    {
        "name": "concinno-daily-reflection",
        "description": (
    ...
    {
        "name": "concinno-weekly-evolution",
        "description": (
...
def handle_status(arguments: dict | None = None) -> str:
    """Execute concinno status and return formatted output."""
    try:
        from concinno.cli.main import MODULES, _load_module_states
    except ImportError:
        MODULES = {}
    ...
    if config_path and os.path.isfile(config_path):
        ...
        lines.append("\n  Config: not found (run `concinno init`)")

    return "\n".join(lines)


def handle_doctor(arguments: dict | None = None) -> str:
    """Execute concinno doctor and return formatted output."""
    lines = ["concinno doctor", ""]
    issues = 0
    hooks_dir = os.path.join(os.path.expanduser("~"), ".claude", "hooks")
    ...
    if issues == 0:
        ...
        lines.append(f"  {issues} issue(s) found. Run `concinno init` to fix.")
...
RESOURCE_HANDLERS = {
    "concinno://session/status": read_session_status,
    "concinno://metrics/quality": read_quality_metrics,
    "concinno://metrics/tokens": read_token_usage,
    "concinno://knowledge/stats": read_knowledge_stats,
}
...
def handle_rag_search(arguments: dict | None = None) -> str:
    ...
    try:
        ...
        if not results:
            return "No results found. Try building the index first: concinno-rag-build"
        lines = []
        for r in results:
        ...
        return "RAG dependencies not installed. Run: pip install concinno[rag]"
    except Exception as exc:
...
def handle_recommendations(arguments: dict | None = None) -> str:
    ...
    fail_path = os.path.join(
        project_dir, ".concinno_cache", "tool_failures.jsonl",
    ) if project_dir else ""
...
def handle_guard_report(arguments: dict | None = None) -> str:
    ...
    audit_path = os.path.join(
        project_dir, ".concinno_cache", "audit", "guard_denies.jsonl",
    ) if project_dir else ""
...
def _do_sync_import(remote: dict, project_dir: str) -> str:
    ...
    if remote_cfg and project_dir:
        cfg_path = os.path.join(
            project_dir, ".concinno_cache", "cc_config.json",
        )
...
def handle_sync_state(arguments: dict | None = None) -> str:
    ...
    if action == "export":
        ...
        cfg_path = os.path.join(project_dir, ".concinno_cache", "cc_config.json")
...
def handle_progress(arguments: dict | None = None) -> str:
    ...
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR", ".")
    cache_dir = os.path.join(project_dir, ".concinno_cache") if project_dir else ""
...
TOOL_HANDLERS = {
    "concinno-status": handle_status,
    "concinno-doctor": handle_doctor,
    "concinno-rag-search": handle_rag_search,
    "concinno-rag-build": handle_rag_build,
    "concinno-recommendations": handle_recommendations,
    "concinno-failure-patterns": handle_failure_patterns,
    "concinno-guard-report": handle_guard_report,
    "concinno-analyze-intent": handle_analyze_intent,
    "concinno-confirm-action": handle_confirm_action,
    "concinno-sync-state": handle_sync_state,
    "concinno-daily-reflection": handle_daily_reflection,
    "concinno-weekly-evolution": handle_weekly_evolution,
    "concinno-screenshot": handle_screenshot,
    "concinno-progress": handle_progress,
    "concinno-cost": handle_cost,
}
...
def _handle_tools_list(req_id: Any, _params: dict) -> dict:
    ...
    mcp = _cfg().raw("mcp", {})
    enabled = mcp.get("tools", ["concinno-status", "concinno-doctor"])
