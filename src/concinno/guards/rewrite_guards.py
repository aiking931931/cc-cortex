"""concinno.guards.rewrite_guards — PreToolUse input rewriters.
...
class BashPipeToShellRewriter(BaseGuard):
    ...
    The rewritten form is ``curl -fsSL URL -o /tmp/concinno-download.sh
    && echo '[concinno] downloaded — inspect before running'``, which
    ...
    def check(self, ctx: GuardContext) -> GuardResult | None:
        ...
        safe_cmd = (
            f"curl -fsSL {url} -o /tmp/concinno-download.sh && "
            "echo '[concinno] downloaded to /tmp/concinno-download.sh — "
        ...
        return GuardResult.rewrite(
            ...
            reason=(
                ...
                "script at /tmp/concinno-download.sh before executing."
            ),
        )
