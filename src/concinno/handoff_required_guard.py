"""concinno.handoff_required_guard — Block stop when session has substantive
...
@dependencies concinno.i18n (handoff_prefixes), concinno.core.config
@exports on_stop
...
Escape valves:
- CONCINNO_FORCE_STOP=1 env (handled at on_stop.py dispatch level — this
...
- CONCINNO_HANDOFF_MINIMAL=1 env — explicit acknowledgment that the
...
def on_stop(hook_data: dict) -> Optional[str]:
    ...
    if not handoff_files:
        ...
        reason = (
            ...
            f"stopping. Escape: set CONCINNO_FORCE_STOP=1 to bypass."
        )
        return f"HANDOFF_REQUIRED_BLOCK:{reason}"

    # Second-layer structural gate.
    # Skipped when disabled via feature config or when the user explicitly
    # acknowledges a minimal update via CONCINNO_HANDOFF_MINIMAL=1.
    if not structural_gate_enabled:
        return None
    if os.environ.get("CONCINNO_HANDOFF_MINIMAL", "").strip() == "1":
        return None
    ...
    reason = (
        ...
        f"(\u2705/\u2B1C/next_step/## section/commit hash/doc link). "
        f"Escape: CONCINNO_HANDOFF_MINIMAL=1 (intentional minimal update) "
        f"or CONCINNO_FORCE_STOP=1 (bypass all)."
    )
    return f"HANDOFF_REQUIRED_BLOCK:{reason}"
