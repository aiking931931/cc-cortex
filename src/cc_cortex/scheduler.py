"""cc_cortex.scheduler — Cross-platform scheduled task launcher for Claude Code.
...
Usage:
  python -m cc_cortex.scheduler self-reflection
  python -m cc_cortex.scheduler scavenger --config /path/to/schedule_config.json
"""
...
logger = logging.getLogger("cc_cortex.scheduler")
...
def _notify(title: str, body: str) -> None:
    """Send a desktop notification via cc_cortex.core.notify (best-effort)."""
    try:
        from cc_cortex.core.notify import show_toast

        show_toast(title, body, tag="cc-scheduler", group="cc-cortex")
    except Exception:
        pass
...
def install_schedule(
    ...
    module_cmd = f"{python} -m cc_cortex.scheduler {task_name}"
...
def _install_launchd(
    ...
    label = f"com.cc-cortex.{task_name}"
...
def _install_cron(task_name: str, cmd: str, interval_minutes: int, work_dir: Optional[str]) -> str:
    ...
    cron_comment = f"# cc-cortex-{task_name}"
    if work_dir:
    ...
    try:
        ...
        # Remove old entry
        lines = [ln for ln in lines if f"cc-cortex-{task_name}" not in ln]
...
def uninstall_schedule(task_name: str) -> str:
    ...
    if SYSTEM == "Windows":
        ...
        try:
            ...
            lines = [
                ln for ln in existing.stdout.splitlines() if f"cc-cortex-{task_name}" not in ln
            ]
            subprocess.run(
                ["crontab", "-"],
                input="\n".join(lines) + "\n",
            ...
            return f"Removed cron: cc-cortex-{task_name}"
        except Exception as e:
            return f"Failed: {e}"
...
def main() -> None:
    """CLI: python -m cc_cortex.scheduler <task_name> [options]"""
    import argparse
