"""cc_cortex.plugin_loader -- Discover, load, and validate guard plugins.
...
@dependencies cc_cortex.guards.base (BaseGuard)
...
1. **Entrypoints** (pip-installed packages):
   ``[project.entry-points."cc_cortex.guards"]``
   ``my_guard = "my_package:MyGuard"``

2. **File paths** (local .py files):
   ``cc-cortex plugins load ./my_guard.py``
...
from cc_cortex.guards.base import BaseGuard
...
def validate_guard(
    ...
    if not isinstance(guard, BaseGuard):
        errors.append(
            ...
            f"All guards must inherit from cc_cortex.guards.BaseGuard."
        )
...
def _import_module_from_path(path: str) -> tuple:
    ...
    module_name = f"_cc_cortex_plugin_{os.path.basename(path)[:-3]}"
...
ENTRYPOINT_GROUP = "cc_cortex.guards"


def _get_entrypoints():
    """Get entrypoints for the cc_cortex.guards group (version-safe)."""
