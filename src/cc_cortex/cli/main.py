"""cc-cortex CLI — init, enable, disable, status, doctor, uninstall."""
...
# (source filename in src/cc_cortex/hooks/, installed filename in ~/.claude/hooks/)
HOOK_FILES = [
...
def _register_hooks_in_settings() -> int:
    """Register cc-cortex hooks in settings.json. Returns count registered."""
...
def _unregister_hooks_from_settings() -> int:
    """Remove cc-cortex hooks from settings.json. Returns count removed."""
...
def cmd_init(args: argparse.Namespace) -> None:
    """Initialize cc-cortex in the current project."""
    print("⚡ cc-cortex init")
    print()
    ...
    print("  Run `cc-cortex doctor` to verify installation.")
    print()
...
def cmd_status(_args: argparse.Namespace) -> None:
    ...
    if cfg_status.exists():
        ...
        print("  Config: not found (run `cc-cortex init`)")


def cmd_doctor(_args: argparse.Namespace) -> None:
    """Health check for cc-cortex installation."""
    print("🩺 cc-cortex doctor")
    print()
    issues = 0
    ...
    if issues == 0:
        ...
        print(f"  {issues} issue(s) found. Run `cc-cortex init` to fix.")
...
def cmd_plugins_scan(_args: argparse.Namespace) -> None:
    ...
    if not metas:
        ...
        print('Packages register via [project.entry-points."cc_cortex.guards"]')
        return
...
def _aegis_context() -> tuple[str, str] | None:
    ...
    if not project_dir or not session_id:
        return None
    cache_dir = os.path.join(project_dir, ".cc_cortex_cache")
...
def cmd_rag_namespaces(_args: argparse.Namespace) -> None:
    """List the RAG namespaces defined in cc_cortex.rag.NAMESPACES."""
...
def cmd_rag_route(args: argparse.Namespace) -> None:
    ...
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR", "")
    cache_dir = (
        os.path.join(project_dir, ".cc_cortex_cache")
        if project_dir
        else os.path.join(os.path.expanduser("~"), ".cache", "cc_cortex")
    )
...
def cmd_uninstall(args: argparse.Namespace) -> None:
    ...
    if not args.yes:
        try:
            answer = input("Remove all cc-cortex hooks and config? [y/N] ")
        except EOFError:
            answer = "n"
    ...
    if removed:
        ...
        print("  ⚠ settings.json — no cc-cortex hooks found")

    print()
    print("  Done! cc-cortex has been removed.")
...
def main() -> None:
    ...
    # init
    p_init = sub.add_parser("init", help="Initialize cc-cortex in current project")
    ...
    # uninstall
    p_uninstall = sub.add_parser("uninstall", help="Remove cc-cortex hooks and config")
