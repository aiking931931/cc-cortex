"""cc_cortex.feature_config — Feature risk metadata, validation, and safe get/set.
...
# how ZIQFeatureRouter (see ``cc_cortex.ziq_router``) treats them:
#
...
def get_active_profile() -> str:
    ...
    Resolution order (first hit wins):
      1. ``CC_CORTEX_PROFILE`` environment variable (if it names a
         known profile)
    ...
    try:
        env_name = _os.environ.get("CC_CORTEX_PROFILE", "").strip()
