"""cc_cortex.boundary_guard — CC/CCC boundary violation detection.
...
def _is_cortex_path(file_path: str) -> bool:
    """Check if file_path is inside the cc_cortex library."""
    normalized = file_path.replace("\\", "/").lower()
    return "cc_cortex/" in normalized or "cc-cortex/" in normalized
    ...
    if _is_hook_path(file_path):
        ...
        if biz_lines > hook_business_threshold:
            return (
                ...
                f"Consider extracting to a cc_cortex module — "
