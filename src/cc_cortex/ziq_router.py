"""cc_cortex.ziq_router — Dynamic feature routing via ZIQ alpha_t classification.

@module ziq_router
@responsibility Decide which cc-cortex features to enable per request based on
...
@dependencies cc_cortex.feature_config, cc_cortex.ziq_retrieval, cc_cortex.core.config
...
if TYPE_CHECKING:  # pragma: no cover
    from cc_cortex.core.config import Config  # noqa: F401
    from cc_cortex.ziq_retrieval import ZIQRetrieval  # noqa: F401
...
# object lives in cc_cortex.core.config.Config and exposes a .feature(name)
...
class ZIQFeatureRouter:
    """Dynamically enable/disable cc-cortex features per request.
    ...
    def _load_meta(self) -> dict[str, dict]:
        ...
        from cc_cortex.feature_config import FEATURE_META
    ...
    def _read_live_value(self, fname: str) -> bool:
        ...
        # Preferred: Config.feature(name) → bool (cc_cortex.core.config.Config)
