"""cc_cortex.sedimentation_gate — Block stop when corrections unsedimented.
...
def _get_session_start_time(session_id: str, workspace: str) -> float:
    ...
    cache_dir = os.path.join(workspace, ".cc_cortex_cache") if workspace else ""
    if cache_dir:
        try:
...
def on_stop(hook_data: dict) -> Optional[str]:
    ...
    Called by cc_cortex.hooks.on_stop pipeline via _build_sedimentation_gate.
    """
