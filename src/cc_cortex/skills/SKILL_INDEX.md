...
CCC built-in skills index — slash commands shipped with `pip install cc-cortex`.
...
| `cortex-hooks` | `cortex hook`, `hook mode`, `鉤子` | Configure cc-cortex hook behavior — switch modes (off/auto/min/max), toggle individual features (destruction_guard, session_lock, token_warn, streak_ux, lint). | `hooks.md` |
