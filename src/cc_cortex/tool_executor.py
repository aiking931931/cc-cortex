"""cc_cortex.tool_executor — Goal-driven multi-step tool execution loop.
...
@responsibility Orchestrate an observe -> think (LLM) -> act (tool call) ->
    ...
    :class:`cc_cortex.core.state_store.StateStore`. Each tool gets
    retry + circuit breaker wrapping.
@dependencies cc_cortex.core.state_store (hard),
    cc_cortex.escalation.LLMEscalator (lazy, optional)
...
logger = logging.getLogger("cc_cortex.tool_executor")
...
def _default_max_concurrency() -> int:
    raw = os.environ.get("CC_CORTEX_MAX_TOOL_USE_CONCURRENCY", "10")
    try:
        n = int(raw)
...
class ToolExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        base_dir = cache_dir or ".cc_cortex_cache"
    ...
    async def run_batched(
        ...
        ``int(os.environ["CC_CORTEX_MAX_TOOL_USE_CONCURRENCY"])`` (or 10).
        ...
        if escalator is None:
            try:
                ...
                msg = (
                    ...
                    "escalator=... or install cc_cortex.escalation"
                )
