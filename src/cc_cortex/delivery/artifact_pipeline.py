"""cc_cortex.delivery.artifact_pipeline — Unified multi-type WIREDO verification.
...
@dependencies cc_cortex.asset_validator, cc_cortex.core.state_store,
    cc_cortex.delivery.wiredo (code-specific helpers)
...
from cc_cortex.asset_validator import AssetType, WiredoDimension, detect_asset_type
...
def _check_code_wired(files: list[str], workspace: str) -> CheckResult:
    ...
    from cc_cortex.delivery.wiredo import _find_unwired_files
...
def _check_code_inherited(files: list[str], workspace: str) -> CheckResult:
    ...
    from cc_cortex.delivery.wiredo import _inherited_check
...
def _check_code_responsive(files: list[str]) -> CheckResult:
    ...
    from cc_cortex.delivery.wiredo import _responsive_check
...
def _check_code_extensible(files: list[str]) -> CheckResult:
    ...
    from cc_cortex.delivery.wiredo import _extensible_check
...
def _check_code_defended(
    ...
    from cc_cortex.delivery.wiredo import _defended_check
...
def _check_code_observable(files: list[str]) -> CheckResult:
    ...
    from cc_cortex.delivery.wiredo import _observable_check
...
class ArtifactPipeline:
    ...
    Usage:
        pipeline = ArtifactPipeline(
            cache_dir="/path/.cc_cortex_cache",
            session_id="abc123",
