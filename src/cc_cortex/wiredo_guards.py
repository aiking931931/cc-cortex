"""cc_cortex.wiredo_guards — WIREDO checklist injection + hard enforcement.
...
@dependencies cc_cortex.guards.base, cc_cortex.asset_validator,
    cc_cortex.delivery.wiredo
...
def _detect_project(workspace: str, file_path: str) -> str:
    ...
    if "aegis" in norm:
        return "aegis"
    if "cc-cortex" in norm or "cc_cortex" in norm:
        return "cc-cortex"
    return ""
...
def _session_has_code_edits(cache_dir: str, session_id: str) -> bool:
    ...
    try:
        from cc_cortex.delivery.wiredo import _get_session_code_files
