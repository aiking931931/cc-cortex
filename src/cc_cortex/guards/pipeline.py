"""cc_cortex.guards.pipeline — Guard execution engine.
...
if TYPE_CHECKING:
    from cc_cortex.equilibrium_breaker import EquilibriumBreaker
...
class GuardPipeline:
    ...
    def _active_profile() -> str:
        ...
        """
        try:
            from cc_cortex.feature_config import get_active_profile
        ...
        for guard in self._guards:
            ...
            try:
                ...
                if guard.category == GuardCategory.SECURITY:
                    ...
                    print(
                        f"[cc-cortex] SECURITY guard {guard.name} crashed: {exc}",
                        file=sys.stderr,
                    )
    ...
    def _get_breaker(self, ctx: GuardContext) -> EquilibriumBreaker | None:
        ...
        if self._breaker is None:
            ...
            from cc_cortex.equilibrium_breaker import EquilibriumBreaker as _TB
    ...
    def _handle_deny(
        ...
        if guard.step_back_reason and self._step_back_dir:
            try:
                from cc_cortex.step_back import wrap_gate
                ...
                print(
                    f"[cc-cortex] step-back failed for {guard.name}: {exc}",
                    file=sys.stderr,
                )
    ...
    def _clear_step_back(self, session_id: str) -> None:
        ...
        if not self._step_back_dir or not session_id:
            return
        try:
            from cc_cortex.step_back import clear_state
    ...
    def _record_failure(self, name: str) -> None:
        ...
        if prev + 1 == self._max_failures:
            try:
                sys.stderr.write(
                    f"\033[93m⚠ [cc-cortex] guard '{name}' disabled after "
