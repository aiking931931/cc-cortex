#!/usr/bin/env python3
"""cc-cortex PostToolUse hook — Guard Pipeline + streak UX + token monitor.
...
def _resolve_transcript_path(session_id: str) -> tuple[str, str]:
    ...
    try:
        from cc_cortex.core.path_utils import (
...
def _token_thresholds(lang: str = "en") -> list[tuple]:
    ...
    try:
        from cc_cortex.token_zone import CHECKPOINT_THRESHOLDS, _parse_model_key, detect_model
...
def _format_token_warning(result: dict, lang: str = "en") -> str:
    ...
    try:
        from cc_cortex.token_zone import detect_model
        profile = detect_model()
...
def _run_knowledge(hook_data: dict) -> str | None:
    ...
    try:
        from cc_cortex.knowledge import on_post_tool as know_post
...
def _run_sentinel(hook_data: dict, tool_name: str) -> None:
    ...
    try:
        from cc_cortex.sentinel import infer_success, record_outcome
...
def _emit_output(fragments: list[str], session_id: str = "") -> None:
    ...
    Goes through write_json_stdout so silent mode (CC_CORTEX_SILENT or
    ...
    if not throttled:
        return
    final_ctx = "\n\n".join(throttled)
    from cc_cortex.hooks.io_utils import write_json_stdout
    write_json_stdout({
        "hookSpecificOutput": {
...
def _run_auto_checkpoint(
    ...
    from cc_cortex.handoff_engine import auto_checkpoint
...
def main(hook_data: dict | None = None) -> None:
    ...
    try:
        if _CACHE_DIR:
            from cc_cortex.thinking_depth_guard import ThinkingDepthGuard
    ...
    try:
        from cc_cortex.prompt_engine import should_reinject
    ...
    try:
        if tool_name == "Read" and _CACHE_DIR:
            ...
            if fp:
                from cc_cortex.ziq_retrieval import record_feedback
