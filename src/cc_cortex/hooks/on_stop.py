#!/usr/bin/env python3
"""cc-cortex Stop hook — async pipeline with per-module timeout + circuit breaker.
...
def _build_auto_delivery(hook_data: dict) -> Callable[[], str | None]:
    def _run() -> str | None:
        from cc_cortex.delivery import auto_delivery_gate
...
def _build_excuse_scanner(hook_data: dict) -> Callable[[], str | None]:
    ...
    def _run() -> str | None:
        from cc_cortex.excuse_scanner import on_stop as excuse_stop
...
def _build_sedimentation_gate(hook_data: dict) -> Callable[[], str | None]:
    ...
    def _run() -> str | None:
        from cc_cortex.sedimentation_gate import on_stop as sed_stop
...
def _build_wiredo_block(hook_data: dict) -> Callable[[], str | None]:
    ...
    def _run() -> str | None:
        ...
        from cc_cortex.delivery.artifact_pipeline import ArtifactPipeline
...
def _build_handoff_claim(hook_data: dict) -> Callable[[], str | None]:
    ...
    def _run() -> str | None:
        from cc_cortex.handoff_claim_guard import on_stop as hcg_stop
...
def _build_handoff_required(hook_data: dict) -> Callable[[], str | None]:
    ...
    def _run() -> str | None:
        from cc_cortex.handoff_required_guard import on_stop as hr_stop
...
def _should_skip_blocks(hook_data: dict) -> bool:
    ...
    2. ``CC_CORTEX_FORCE_STOP=1`` env var — user escape valve for manual
       override (e.g., `CC_CORTEX_FORCE_STOP=1 claude` then /logout).
    """
    if hook_data.get("stop_hook_active"):
        return True
    return os.environ.get("CC_CORTEX_FORCE_STOP") == "1"
...
def _has_active_delivery() -> bool:
    ...
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR", ".")
    state_path = os.path.join(project_dir, ".cc_cortex_cache", "delivery_state.json")
...
def _orphan_scan(hook_data: dict) -> None:
    ...
    from cc_cortex.delivery import scan_orphans_batch
...
def _notify_stop(hook_data: dict) -> None:
    ...
    git_line = ""
    try:
        from cc_cortex.git_assist import auto_commit, generate_report
...
def _kill_mcp_children(cli_pid: int, all_procs: list, proc_map: dict) -> int:
    ...
    from cc_cortex.process_guard import _get_child_tree
...
def _cleanup_mcp_servers() -> None:
    ...
    try:
        from cc_cortex.process_guard import _get_all_processes
    except ImportError:
        return
