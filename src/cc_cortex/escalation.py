"""cc_cortex.escalation — Multi-tier LLM gateway with auto-fallback.
...
Design:
  ...
  ``import cc_cortex.escalation`` stays cheap and does not hard-depend on
...
class LLMEscalator:
    ...
    Args:
        ...
        cache_dir: Directory for persistent circuit-breaker state. If
            ``None`` a default under ``~/.cc_cortex/escalation`` is used.
    ...
    def __init__(
        ...
        *,
        ...
        self._cache_dir = cache_dir or os.path.join(
            os.path.expanduser("~"), ".cc_cortex", "escalation"
        )
    ...
    def _call_gemma(
        ...
        *,
        ...
        base_url = os.environ.get(
            "CC_CORTEX_GEMMA_URL", "http://localhost:8400/v1"
        )
        api_key = os.environ.get("CC_CORTEX_GEMMA_KEY", "")
        model = os.environ.get("CC_CORTEX_GEMMA_MODEL", "gemma-4-27b")
...
def _anthropic_model_for(tier: EscalationTier) -> str:
    if tier == "haiku":
        return os.environ.get("CC_CORTEX_HAIKU_MODEL", "claude-haiku-4-5")
    if tier == "sonnet":
        return os.environ.get("CC_CORTEX_SONNET_MODEL", "claude-sonnet-4-6")
    if tier == "opus":
        return os.environ.get("CC_CORTEX_OPUS_MODEL", "claude-opus-4-6")
