"""cc_cortex.cache.autocompact — Last-resort context auto-compaction.
...
@responsibility Port of Claude Code's ``autoCompact`` layer. When a
    ...
    :class:`~cc_cortex.cache.microcompact.CacheEditSink`).
...
- The TS source threads ``AutoCompactTrackingState`` through the query
  ...
  and persisted via :class:`~cc_cortex.core.state_store.StateStore`.
...
logger = logging.getLogger("cc_cortex.cache.autocompact")
...
class AutoCompactState:
    ...
    Persisted via :class:`~cc_cortex.core.state_store.StateStore` so a
