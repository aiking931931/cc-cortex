"""cc_cortex.cache.memdir — Append-only dated event log under MEMORY.md.
...
@responsibility Python port of Claude Code's ``memdir`` subsystem
    ...
    **durable, structurally append-only** layer that sits underneath
    :mod:`cc_cortex.cache.session_memory`.

    Think of the pair as an index + log:

    - ``MEMORY.md`` (the "entrypoint" / index) is what
      :class:`~cc_cortex.cache.session_memory.SessionMemory` distills
    ...
    memdir into :class:`cc_cortex.ziq_retrieval.ZIQRetrieval` as a
    namespace.
    ...
    4. **No personal paths**. Default root is
       ``~/.claude/.cc_cortex_memdir``; ``CC_CORTEX_MEMDIR`` env var
       overrides.
...
logger = logging.getLogger("cc_cortex.cache.memdir")
...
def _default_root() -> Path:
    ...
    Honours ``CC_CORTEX_MEMDIR`` when set, otherwise falls back to
    ``~/.claude/.cc_cortex_memdir``. NEVER hardcodes a workspace path:
    CCC is a library, not an application.
    """
    env = os.environ.get("CC_CORTEX_MEMDIR")
    if env:
        return Path(env)
    return Path.home() / ".claude" / ".cc_cortex_memdir"
...
class Memdir:
    ...
    def entrypoint_path(self) -> Path:
        ...
        :class:`~cc_cortex.cache.session_memory.SessionMemory` or an
    ...
    def find_relevant(
        ...
        *,
        ...
        — use :class:`cc_cortex.ziq_retrieval.ZIQRetrieval` when you
    ...
    def truncate_entrypoint(
        ...
        *,
        ...
        Mirrors :meth:`cc_cortex.cache.session_memory.SessionMemory.truncate_content`
