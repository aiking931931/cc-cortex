"""cc_cortex.cache.cognitive_pool — L3 shared markdown pool with stable section hashes.

@module cache.cognitive_pool
@responsibility L3 of the ZIQ v7 three-layer cognitive sharing
    architecture. Sits above :mod:`cc_cortex.cache.session_memory`
    (per-session) and :mod:`cc_cortex.cache.memdir` (cross-session
    ...
    5. **No personal paths**. Default root is
       ``~/.claude/.cc_cortex_pool``; ``CC_CORTEX_POOL_ROOT`` env var
...
logger = logging.getLogger("cc_cortex.cache.cognitive_pool")
...
#: ``CC_CORTEX_POOL_ROOT`` environment variable.
...
def _default_root() -> Path:
    ...
    Honours ``CC_CORTEX_POOL_ROOT`` when set, otherwise falls back to
    ``~/.claude/.cc_cortex_pool``. NEVER hardcodes a workspace path:
    CCC is a library, not an application.
    """
    env = os.environ.get("CC_CORTEX_POOL_ROOT")
    if env:
        return Path(env)
    return Path.home() / ".claude" / ".cc_cortex_pool"
