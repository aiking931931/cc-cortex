"""cc_cortex.cache.session_memory — Persistent session memory distiller.
...
@responsibility Port of Claude Code's ``services/SessionMemory`` layer.
    ...
    recover state after :mod:`~cc_cortex.cache.microcompact` deletes old
    ...
    - **state**: :class:`SessionMemoryState` persisted via
      :class:`~cc_cortex.core.state_store.StateStore` so tool counts
    ...
    distillation pass an **isolated** :class:`~cc_cortex.agent.fork_context.SubagentContext`
...
@dependencies cc_cortex.core.state_store, cc_cortex.agent.fork_context
...
from cc_cortex.agent.fork_context import SubagentContext

logger = logging.getLogger("cc_cortex.cache.session_memory")
...
class DistillSink(Protocol):
    ...
    :class:`~cc_cortex.agent.fork_context.SubagentContext` for the
...
def _default_cache_dir() -> Path:
    ...
    Uses the ``CC_CORTEX_SESSION_MEMORY_DIR`` env var when set, so
    hosts / tests can redirect storage without monkeypatching.
    """
    env = os.environ.get("CC_CORTEX_SESSION_MEMORY_DIR")
    if env:
        return Path(env)
    return Path.home() / ".claude" / ".cc_cortex_session_memory"
...
class SessionMemory:
    ...
    Args:
        cache_dir: Root directory for the markdown file and persisted
            ...
            ``$CC_CORTEX_SESSION_MEMORY_DIR`` or
            ``~/.claude/.cc_cortex_session_memory``.
        ...
        fork_context: Optional isolated
            :class:`~cc_cortex.agent.fork_context.SubagentContext`
    ...
    def fork_context(self) -> SubagentContext | None:
        ...
        the :class:`~cc_cortex.agent.fork_context.FileStateCache`
