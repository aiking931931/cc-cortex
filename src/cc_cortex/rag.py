"""cc_cortex.rag — Cognitive RAG: AI self-evolution memory, not Q&A.
...
from cc_cortex.anti_bloat import HitTracker, Pruner, StaleDetector
...
class RAGIndex:
    ...
    Args:
        ...
        project_dir: Project root directory.
        cache_dir: Where to store the vector DB. Default: .cc_cortex_cache/rag
        model_name: Sentence-transformers model for embeddings.
        collection_name: ChromaDB collection name.
        include_bundled: Whether to include cc-cortex's bundled knowledge/
            directory (cognitive frameworks shipped with the package).
    """

    # Bundled cognitive frameworks shipped with cc-cortex
    _BUNDLED_KNOWLEDGE_DIR: str = ""

    @classmethod
    def _bundled_knowledge_dir(cls) -> str:
        """Return absolute path to cc-cortex's bundled knowledge/ directory."""
        if not cls._BUNDLED_KNOWLEDGE_DIR:
            # knowledge/ lives at package root (sibling of src/)
            pkg_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            # Go up from cc_cortex/ → src/ → cc-cortex/ → knowledge/
    ...
    def __init__(
        ...
        self.cache_dir = cache_dir or os.path.join(
            self.project_dir, ".cc_cortex_cache", "rag"
        )
