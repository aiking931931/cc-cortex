"""cc_cortex.agent.retrieve_pipeline — ZIQ cascade pipeline orchestrator.

@module agent.retrieve_pipeline
@responsibility Glue layer between the three-layer cascade retriever
    (:class:`~cc_cortex.agent.iterative_retrieve.IterativeRetriever`)
    and the FTRL source-weight reranker
    (:class:`~cc_cortex.ziq_retrieval.ZIQRetrieval`).
...
@dependencies cc_cortex.agent.iterative_retrieve, cc_cortex.ziq_retrieval
...
if TYPE_CHECKING:
    ...
    from cc_cortex.cache.l2_distill import RawHit
...
class ZIQCascadePipeline:
    ...
    Example:
        ...
        >>> pipeline.feedback(["src/cc_cortex/cache/cache_break_detector.py"])
    """
