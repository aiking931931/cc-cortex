"""cc_cortex.core.path_utils — Unified path extraction, normalization, and transcript lookup.
...
def _cache_path() -> str:
    ...
    if not project_dir:
        return ""
    return os.path.join(project_dir, ".cc_cortex_cache", _CACHE_FILENAME)
...
def find_transcript(session_id: str) -> str:
    ...
    Results are cached to ``<project>/.cc_cortex_cache/transcript_path.txt``.
    """
    if not session_id:
        return ""
