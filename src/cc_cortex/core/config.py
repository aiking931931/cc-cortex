"""
cc_cortex.core.config — Central configuration loader.
...
Usage:
    from cc_cortex.core import get_config
...
class Config:
    ...
    def toast_tag(self) -> str:
        self._load()
        return self._data.get("notification", {}).get("toast_tag", "cc-cortex")

    @property
    def toast_group(self) -> str:
        self._load()
        return self._data.get("notification", {}).get("toast_group", "cc-cortex")
...
def get_config(hooks_dir: Optional[str] = None, config_path: Optional[str] = None) -> Config:
    ...
    if _instance is None:
        ...
        if hooks_dir and hooks_dir != _instance._hooks_dir:
            logging.getLogger("cc_cortex").warning(
