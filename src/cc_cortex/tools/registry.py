"""cc_cortex.tools.registry — deferred tool registry with lazy-load search.
...
@responsibility Split tool surface into "core" (always in prompt) and
    ...
    long session.
@dependencies cc_cortex.tool_executor.Tool (Protocol, runtime_checkable)
...
logger = logging.getLogger("cc_cortex.tools.registry")
...
class ToolEntry:
    ...
    entry-point specs, e.g. ``"cc_cortex.tools.builtin.shell:Shell"``.
...
class ToolRegistry:
    ...
    Usage::
        ...
        reg.register_deferred(
            name="Shell",
            import_path="cc_cortex.tools.builtin.shell:Shell",
