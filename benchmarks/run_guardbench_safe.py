"""Fault-tolerant GuardBench runner — skips failed datasets."""
import json
import sys
import traceback
...
from cascade_guard import moderate  # noqa: E402

MODEL_ID = "ibm-granite/granite-guardian-hap-125m"
MODEL_NAME = "cascade-granite-hap-125m"
...
DATASETS = [
    ...
    # Skipped: advbench_behaviors, advbench_strings (GitHub timeout)
    # Skipped: decoding_trust_stereotypes (gated, needs HF_TOKEN)
]
...
for i, ds in enumerate(DATASETS):
    ...
    try:
        guardbench.benchmark(
            ...
            out_dir="/workspace/gb_results_v2",
            batch_size=32,
            datasets=[ds],
...
print(f"\n{'=' * 60}")
print(f"DONE: {len(DATASETS) - len(failed)}/{len(DATASETS)} succeeded")
print(f"Failed: {failed}")
with open("/workspace/gb_summary_v2.json", "w") as f:
