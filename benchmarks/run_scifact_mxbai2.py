"""SciFact: mxbai-rerank-large-v2 with correct loader."""
import json
import os
...
def main():
    ...
    # Load mxbai-rerank-large-v2 (correct loader)
    print("Loading mxbai-rerank-large-v2...")
    ...
    for mname, source in methods.items():
        ...
        for qi, qid in enumerate(qids):
            ...
            doc_texts = [get_text(corpus, did) for did, _ in cands]
            dids = [did for did, _ in cands]

            ranked = reranker.rank(
                queries[qid], doc_texts,
                return_documents=False,
            )
            # MxbaiRerankV2.rank returns list of RankResult
            for item in ranked:
                run.setdefault(qid, {})[dids[item.index]] = float(
                    item.score
                )
            if (qi + 1) % 100 == 0:
    ...
    sep = "=" * 50
    print(f"\n{sep}")
    print("SCIFACT mxbai-rerank-large-v2 (correct loader)")
    ...
    with open("scifact_mxbai2_results.json", "w") as f:
